...
#取運行中服務的image版號
runningno1=`cat ./$txtRunningServicePath/Running.txt | grep im_im-$servicename |  awk -F ':' '{print $2}'  | awk '{print $1}' | awk 'NR==1{print}'`
runningno2=`cat ./$txtRunningServicePath/Running.txt | grep im_im-$servicename |  awk -F ':' '{print $2}'  | awk '{print $1}' | awk 'NR==2{print}'`
#運行的服務名稱
runing1=`cat ./$txtRunningServicePath/Running.txt | grep im_im-$servicename  | awk '{print $2}' | awk 'NR==1{print}'`
runing2=`cat ./$txtRunningServicePath/Running.txt | grep im_im-$servicename  | awk '{print $2}' | awk 'NR==2{print}'`

if [ $runningno1 = $runningno2  ];then
runningno=$runningno1
else
echo -e "\033[33m $runing1 與 $runing2 所運行的image不同!! \033[0m"
    exit 2
fi

echo  -e "\033[37m目前的 Y M L 版號 =>\033[0m" "\033[44m$newservicename:$ymlno\033[0m"
echo  -e "\033[37m目前$runing1 的Running版號 =>\033[0m" "\033[42m$newservicename:$runningno\033[0m"
echo  -e "\033[37m目前$runing2 的Running版號 =>\033[0m" "\033[42m$newservicename:$runningno\033[0m"
    #運行與yml版本號判斷-in
...
sed -i s/"$ymlno"/"jm_pro_1.$no"/g  $path/$servicename.yml
...
sleep 2
echo -e "\033[36m * 清cloudflare Cache * \033[0m"
sh /root/cloudflare-cli/cache-clear.sh
echo -e "\033[32m * 清cloudflare Cache成功 * \033[0m"
sleep 2
#clear cache
#site 分類
...
/bin/ssh -p 6343 op@im-test-001 rectn test  rm -rf /tmp/proxy-cache/*
...
elif [ $servicename = file ] || [ $servicename = robot ] || [ $servicename = smarttalk ] || [ $servicename = gateway ] ||  [ $servicename = api ] ;then
