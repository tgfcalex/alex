    ...
    if echo $RSPKG | grep -wo $i &>/dev/null && [[ $i != "api"  ]] && [[ $i != "merchant-api" ]] && [[ $i != "distributor-api"  ]] && [[ `ls -A /var/env/apps/lb/work/$i` != "" ]];then
        if [[ $host == "la3" && $project == "lb" ]] || [[ $host == "lr1" && $project == "lb" ]];then
            if [[ $i == "rcenter" ]];then
                ...
                command_with_info
                #添加一个以备份时间命名的文件
                /usr/bin/touch  ${bak_app_dir}/${host}/${i}/bakTime_$(date +%F-%H:%M:%S)
                echo -e "静态资源全线同步 ==> ${i}"
                command="rsync -azq  ${latest_app_dir}/$i/ ${distributed_dir}/$i/${dubbo_version}/$i/"
                command_with_info
            else
               for SERVER in ${RS};do
                    echo -e "${SERVER}备份==>${i}"
                ...
                echo -e "静态资源全线同步 ==> ${i}"
                command="rsync -azq ${latest_app_dir}/$i/  ${distributed_dir}/$i/"
                command_with_info
            fi
        fi
...
#推送tomcat的包执行此命令
        ...
        if [[ $host == "la1" || $host == "la2" || $host == "la3" ]];then
           #备份远程主机的app包到控制机
            ...
            #添加一个以备份时间命名的文件
            touch ${bak_app_dir}/${host}/${i}/bakTime_$(date +%F-%H:%M:%S)
