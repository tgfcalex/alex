...
#推送静态资源的包（增量)
    if echo $RSPKG | grep -wo "$i" &>/dev/null  && [[ `ls -A /var/env/apps/lb/work/$i` != "" ]];then
        if [[ $host == "la1" && $project == "v-lb" ]] || [[ $host == "lr1" && $project == "v-lb" ]];then
            if [[ $i == "rcenter" ]];then
                echo -e "rcenter推送 ==> ${i}"
                command="ansible  r  -m synchronize -a 'src=${latest_app_dir}/work/$i/ dest=${v_dist_app_dir}/$i/${dubbo_version}/$i/ delete=yes'"
                command_with_info
            else
                echo -e "静态资源全线同步 ==> ${i}"
                command="ansible  r  -m synchronize -a 'src=${latest_app_dir}/work/$i/ dest=${v_dist_app_dir}/$i/ delete=yes'"
                command_with_info
            fi
        fi
...
#推送tomcat的包执行此命令
        ...
        if [[ $host == "la1" ]];then
            echo -e "${host}同步${i}"
            command="sudo /usr/bin/rsync -azq   ${latest_app_dir}/${i}/ ${username}@${host}:${v_dist_app_dir}/${i}/ delete=yes"
            command_with_info
        else
