...
# 使用帮助
    if [ $# -ne 1 ] || [[ $1 == "--help" ]];then
        ...
        ---------------------------------------------------------------
            la{1..3} ==> 推送tomcat的包
            la3 ==>  当输入la3时，所有静态资源包全线推送{lr1,lr2,lr3}
        ---------------------------------------------------------------
...
_enter_ok_go_on
source $cur_dir/../../../common/script/release/service_stop_new.sh $line $options_all
...
if [[ $line == la3 && $project == "lb" ]];then
    _h2 "[ Step ${ITEM}.${ITEM_EX} ]: 推送所有r静态资源 ==> 全线 ${i} "; let ITEM_EX+=1
    _enter_go_on
    source $cur_dir/app_update_new.sh $line  $RSPKG
fi
